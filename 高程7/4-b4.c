...
int menu() {
	...
	printf("3.用箭头键控制上下左右，边界停止(按HPKM时不准移动)\n");
	printf("4.用箭头键控制上下左右，边界回绕(按HPKM时不准移动)\n");
...
int main()
{
 ...
 /*
 思考
 1.方向键返回两个键值，按左方向键键时，第一次用_getch()读到224， 程序不做反应（default）， 第二次读到75,与K的键值相同，故向下移
 2.不一定能生成20个字母，因为可能在同一位置重复生成字母 
